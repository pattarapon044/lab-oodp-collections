...
public class PancakeShop {
	...
	public void serveLunch() {
		...
		System.out.println("We have " + customerQueue.size() + " customers today, and have made " + pancakeStack.size()
				+ " pancakes. Hope it's enough!");
		...
		while (customerQueue.size() > 0) {
			...
			System.out.println(customer.getName() + " sat at the table. They want to eat " + customer.getBellySize()
					+ " pancakes for lunch!");
	...
	private Deque<Pancake> createPancakes() {
		...
		Deque<Pancake> pancakes = new ArrayDeque<Pancake>();
